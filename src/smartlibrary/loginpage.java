...
import static smartlibrary.homepage.text1;
import static smartlibrary.homepage.text2;
...
public class loginpage extends JFrame implements ActionListener, MouseListener {
    ...
    public static void addUser(String name, String password) {
        if (manuser.ind == -1) {
            ...
            for (int i = 0; i < users.size(); i++) {
                if (text1.getText().equals(users.get(i).getUsername())) {
                    f = false;
    ...
    public static void editUser(String name, String pass, int i) {
        if (!(i == -1)) {
            ...
            for (int j = 0; j < loginpage.users.size(); j++) {
                if (text1.getText().equals(loginpage.users.get(j).getUsername()) && text2.getText().equals(users.get(i).getPassword())) {
                    f = false;
                    JOptionPane.showMessageDialog(null, "UserName Already Taken", "Warnning", JOptionPane.ERROR_MESSAGE);
                    break;
                }
